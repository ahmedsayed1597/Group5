...
export class ProductService {
  ...
  public addProduct(categoryId: number, product: any, image: File) {
    ...
    return this._httpClient.post<ProductToAdd>(
      `http://localhost:9090/api/admin/categories/${categoryId}/product`,
      formData
    );
  }
  ...
  public searchProductsByKeyword(
    ...
    return this._httpClient.get<ResponseViewModel>(
      `http://localhost:9090/api/products/keyword/` +
        keyword +
        `?pageNumber=${pageNumber}&pageSize=${pageSize}&sortBy=${field}&sortOrder=${orderBy}`
    );
  }

  public editProduct(productId: number, editedProduct: ProductToAdd) {
    return this._httpClient.put<ProductViewModel>(
  ...
  public getProductById(id: number) {
    return this._httpClient.get<ProductToAdd>(
  ...
  public addCategory(categoryToAdd: CategoryToAdd) {
    ...
    return this._httpClient.post<CategoryView>(
      'http://localhost:9090/api/admin/categories',
      categoryToAdd,
      { headers: head }
    );
  }
